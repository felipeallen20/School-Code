...
export default function Exercise({ courseSlug, lessonSlug }: ExerciseProps) {
  ...
  const { complete } = useCourseProgress(courseSlug);
  ...
  }, [running, entries, exercise, code, complete, lessonSlug]);
  ...
  return (
    <section className="my-10 overflow-hidden rounded-card border border-border bg-surface">
      <iframe
        ref={iframeRef}
        title="Entorno de ejecución del ejercicio"
      ...
      <div className="flex items-center gap-2 border-b border-border bg-surface px-4 py-2">
        ...
        <span className="text-sm font-medium text-text">Ejercicio</span>
      </div>
      <div className="px-4 py-4">
        <p className="text-sm leading-relaxed text-text">{exercise.instruction}</p>
      </div>
      <CodeEditor
        value={code}
        onChange={handleEdit}
        label="Editor del ejercicio"
